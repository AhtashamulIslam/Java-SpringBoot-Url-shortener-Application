...
const Navbar = () => {
  ...
  const handleLogout = () => {
      setToken(null) ; // First in our context I make it null;
  ...
  return (
    <div className="h-16 bg-blue-600  z-50 flex items-center sticky top-0 ">
      <div className="lg:px-14 sm:px-8 px-4 w-full flex justify-between">
        ...
        <ul
          className={`flex sm:gap-10 gap-4 sm:items-center sm:mt-1 sm:pt-0 text-slate-800 sm:static absolute left-0 top-[62px] sm:shadow-none shadow-md ${
            navbarOpen
              ? "h-fit sm:pb-0 pb-5 max-sm:bg-blue-400 pt-3"
              : "h-0 overflow-hidden"
          }  transition-all duration-100 sm:h-fit sm:w-fit w-full sm:flex-row flex-col px-4 sm:px-0`}
        >
          <li className="hover:text-green-800 font-[500]  transition-all duration-150">
            <Link
              className={`${
